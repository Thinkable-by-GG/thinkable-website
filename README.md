...
Since forms version `2026-09-15-forms-4` submissions are stored in **Flamingo → Inbound Messages**, filtered by a honeypot +
heuristics (+ Cloudflare Turnstile when keys are configured), and optionally forwarded to partner-api's lead ingest. Email
notification is a switch in **Settings → Thinkable Forms** and stays off until the host gets an SMTP relay (it has no mail transport).
